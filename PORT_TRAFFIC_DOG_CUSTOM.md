...
- 远程自定义地址必须使用 `https://example.com` 形式；`http://` 只允许 `localhost`、`127.0.0.1` 和 `[::1]`；
- 也兼容以 `/bot` 结尾的地址拼接。
...
- 依赖补齐（含 `curl`、`tzdata`、`conntrack-tools`）；
...
- 配置导入会校验归档内容、业务字段和端口重叠，并保留临时回滚副本；计数、配额、限速或 cron 任一步恢复失败都会恢复原配置与运行状态。
- 有监控端口时维护 `@reboot` 运行时恢复任务，重启后从 `traffic_data.json` 恢复 counter，再重建 quota、计数和 TC 规则。
- 每分钟自然日快照采用一次原子写入，同时刷新 counter 灾备并按保留天数裁剪历史，避免端口越多时重复改写整份 JSON。
- 端口段只在启用 TC 限速时使用独立 mark；仅占用高 20 位并保留低 12 位，普通统计规则不改写 mark。
- 自动重置按“端口 + 到期日期”记录完成状态，配置日期推进失败时不会重复清零。
- counter 与 quota 通过单个 nftables 事务以零值重建；重置后立即到达的新流量归入新周期，不再触发重复重置。
- root crontab 的读改写使用独立锁，读取异常时不会写回空任务表。
- 更新和迁移在覆盖前校验旧配置，并在后续维护失败时自动回滚脚本、模块、配置和定时任务。
- 脚本创建的 TC 根队列会记录本机归属；未知归属的 HTB 不会被接管，最后一个限速移除后仅在队列确实空闲时自动清理。
