...
restore_monitoring_if_needed() {
    ...
    for port in "${active_ports[@]}"; do
        runtime_status=0
        port_runtime_rules_complete "$port" || runtime_status=$?
...
dog_tc_status_kind() {
    case "${1:-}" in
        TC_STATUS=OK*) echo "ok" ;;
        TC_STATUS=IDLE*) echo "idle" ;;
        TC_STATUS=CONFLICT*) echo "conflict" ;;
        *) echo "error" ;;
    esac
}
...
manage_tc_recovery() {
    ...
    echo
    echo "1. 检测并按需重建 Dog/NTC 规则"
    echo "2. 检测并按需重建，同时启用开机自动恢复"
    echo "3. 启用开机自动恢复"
    echo "4. 关闭开机自动恢复"
    echo "0. 返回主菜单"
    echo
    ...
    case "$tc_choice" in
        1|2)
            status_output=$(dog_tc_status 2>/dev/null || true)
            status_kind=$(dog_tc_status_kind "$status_output")
            case "$status_kind" in
                ok)
                ...
                conflict)
                    ...
                    ;;
                *)
                    echo -e "${RED}无法确认当前 TC 状态，未修改 qdisc。请先运行 dog --self-check。${NC}"
                    ;;
            esac
            ;;
        3)
...
export_config() {
    ...
    case "$active_ports_status" in
        0)
            if ! record_traffic_snapshot >/dev/null 2>&1 ||
               ! save_traffic_data >/dev/null 2>&1; then
        ...
        1) ;;
        *)
            echo -e "${RED}配置包导出失败：无法读取当前端口配置${NC}"
            read -p "按回车键返回..."
            manage_configuration
            return 1
            ;;
    ...
    if [ $# -gt 0 ]; then
        case $1 in
            ...
            --snapshot-traffic)
                ...
                local nft_runtime_status=0
                runtime_nft_rules_complete || nft_runtime_status=$?
                case "$nft_runtime_status" in
                    0) ;;
                    1) restore_runtime_state false >/dev/null 2>&1 || exit 1 ;;
                    *) exit "$nft_runtime_status" ;;
                esac
                record_traffic_snapshot >/dev/null 2>&1
                exit $?
                ;;
